...
window.MgApi = {
   ...
   defaultApiPageSize: 15,
    ...
    _callMethod: function(name, args)
    {
        ...
        var ajaxCall = $.ajax({
            url: ((((window.location.host == "memegenerator.net") || (window.location.host.indexOf("localhost:18009") != -1)) ? ("/Version1Api/") + name : ("https://api.memegenerator.net/" + name)) + ("?u=" + (new Date().valueOf()))),
            type: "GET",
            crossDomain: true,
            data: args2,
            success: function(e) {
...
Instance_Create: function(languageCode, generatorID, groupID, imageID, text0, text1, tags)
{
    return MgApi._callMethod("Instance_Create", { languageCode: languageCode, generatorID: generatorID, groupID: groupID, imageID: imageID, text0: text0, text1: text1, tags: tags });
},
...
Instances_Select_ByPopular: function(languageCode, pageIndex, urlName, tag, days)
{
    return MgApi._callMethod("Instances_Select_ByPopular", { languageCode: languageCode, pageIndex: pageIndex, urlName: urlName, tag: tag, days: days });
},
