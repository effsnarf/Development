...
class SeoArticles {
  ...
  async generateArticle(
    ...
    if (sections && !Array.isArray(sections))
      throw new Error(
        `[sections] must be an array of numbers (not ${JSON.stringify(
          sections
        )})`
      );
    ...
    if (sections)
      prompts.push(
        `The article should contain ${
          sections.length
        } sections. The length of each section should be: ${sections
          .map((s) => `${s} words`)
          .join(", ")}.`
      );
    ...
    prompts.push(
      '{ "title": "[article title]", "sections": [ {"title": "..", "text": "    .."}, {"title": "..", "text": ".."}, ... ], "image": { "keywords": [array of string keywords for image search query that would fit the article] } }'
    );

    const article = Objects.json.parse(await chat.send(prompts.join("\n")));

    // Sometimes the AI fucks up, so we try again
    if (article.sections.length != sectionsCount)
    ...
    console.log(
      `Searching for images (${article.image.keywords.join(", ")})..`
    );

    const imageUrls = await Pexels.searchImages(
      article.image.keywords.join(" "),
      12
    );
