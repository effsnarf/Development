...
function LocalPersistedObject(key, defaultValue) {
  this.saveTimer = null;
  this.key = key;
  var savedItem = storage.inst.getItem(this.key);
  this._value = !savedItem ? defaultValue : JSON.parse(savedItem);
  this.save = () => {
    clearTimeout(this.saveTimer);
    this.saveTimer = setTimeout(() => {
      storage.inst.setItem(this.key, JSON.stringify(this._value));
    }, 100);
  };
  ...
  this.createProxyMethod = (obj, name) => {
    obj[name] = (...args) => {
      var result = this.value[name](...args);
      this.save();
      return result;
    };
  };
}
function LocalPersistedArray(key) {
  this.items = new LocalPersistedObject(key, []);
  this.items.createProxyMethod(this, "push");
  this.items.createProxyMethod(this, "splice");
