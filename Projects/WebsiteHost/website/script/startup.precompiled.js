        ...
        this.fetchAsJson =
            _fetchAsJson ||
                (async (url, ...args) => {
                    ...
                    const text = await (0, Extensions_Network_1.fetchWithAlertify)(url, args[0]);
                    if (!text?.length)
                        return null;
...
Object.defineProperty(exports, "__esModule", ({ value: true }));
exports.fetchWithAlertify = exports.fetchWithProgress = void 0;
// New fetch method with progress update callback
const fetchWithProgress = async (url, options, updateProgress) => {
...
const fetchWithAlertify = async (url, options) => {
    ...
    const result = await fetchWithProgress(url, options, (received, total) => {
        if (["memegenerator.net"].some((host) => window.location.hostname.includes(host)))
            return;
...
exports.fetchWithAlertify = fetchWithAlertify;
...
class Objects {
    ...
    static async awaitWithTimeout(func, milliseconds) {
...
(function (CharType) {
    ...
    CharType["Unicode"] = "unicode";
    CharType["ColorCode"] = "colorCode";
})(CharType || (CharType = {}));
...
class Character {
    ...
    static fromNextCharacter(str) {
        if (!str)
            return null;
        for (const regexKey in Character.regex) {
            const regex = Character.regex[regexKey];
            const match = str.match(regex);
            if (match)
                return Character.new(match[0]);
        }
        return Character.new(str[0]);
    }
    static getStringChars(str) {
        const chars = [];
        let c = null;
    ...
    static detectType(char) {
        for (const regexKey in Character.regex) {
            const regex = Character.regex[regexKey];
            if (char.match(regex)) {
                const charType = regexKey.parseEnum(CharType);
                if (!charType)
                    throw new Error(`Invalid CharType: ${regexKey}`);
                return charType;
            }
        }
...
class String1 {
    ...
    shorten(maxLength, ellipsis = true) {
        const chars = [...this.chars];
        while (chars.length > maxLength) {
            // Find the index of the last non-control character
            const lastNonControlCharIndex = chars.findIndexFromEnd((c) => !controlCharacterTypes.includes(c.type));
            if (lastNonControlCharIndex < 0)
                break;
    ...
    static shorten(str, maxLength, ellipsis = true) {
...
if (typeof String !== "undefined") {
    ...
    String.prototype.includesWholeWord = function (word) {
...
if (typeof Array !== "undefined") {
    ...
    Array.prototype.findIndexFromEnd = function (predicate) {
...
/*!*********************************************************!*\
  !*** ../../WebsiteHost/website/script/1709816606899.ts ***!
  \*********************************************************/
