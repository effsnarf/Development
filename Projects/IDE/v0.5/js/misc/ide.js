            ...
            method: {
                enter: (methodPath) => {
                    ...
                    if (false)
                    {
                        ...
                        if (!method.comp().name.startsWith(`IDE.`))
                        {
                            var fullName = `${method.comp().name}.${method.name}`;
                            console.warn(`enter - ${fullName}`);
                        }
                    }
                },
                exit: (methodPath) => {
                    ...
                    if (depth.depth == 0)
                    {
                        ...
                        if (item)
                        {
                            ...
                            if (showWarning) console.warn(`${util.number.commas(elapsed)}ms`, `(${depth.maxDepth} calls deep)`, methodName);
                        }
                        depth.maxDepth = 0;
                    }
                }
