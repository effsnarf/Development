...
var compDom = {
  ...
  fix: {
    ...
    styles: (comp) => {
      ...
      for (var style of comp.styles)
      {
        if (!(`global` in style)) style.global = false;
      }
    }
  },
  item: {
  ...
  get: {
    ...
    item: (compID, itemID) => {
      ...
      var node = comp.nodes.find(n => (n.id == itemID));
      if (node) return { node: util.with(node, `comp`, comp) };
    ...
    node: {
      ...
      title2: (node) => {
        ...
        var vmodel = node.attrs.find(a => (a.name == "v-model"))?.value;
        if (vmodel) title2 = `⦗${vmodel.shorten(shortLength)}⦘`;
        var vtext = node.attrs.find(a => (a.name == "v-text"))?.value;
        if (vtext) title2 = `⦗${vtext.shorten(shortLength)}⦘`;
    ...
    comp: {
      ...
      related: {
        items: async (comp) => {
          ...
          nodes = nodes.filter(n => (n.tag == comp.name.kebabize()));
          items.push(...nodes.map(compDom.to.item));
          // contains
          var nodes = compDom.get.all.nodes(comp);
