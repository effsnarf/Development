...
class NodePath {
    ...
    toString() {
        let s = this.nodes.map((node) => Utility.getNodeIconDesc(node, this.tree)).join(`   `);
...
class PersistedTree extends PersistedSomething {
    ...
    getNodePath(_id: any): Node[] {
        if (typeof (_id) != `number`) throw new Error(`getNodePath: _id must be a number`);
    ...
    getNodePaths(currentPath?: NodePath): NodePath[] {
        if (!currentPath) return this.getNodePaths(NodePath.fromNode(this, this.root.model));
    ...
    async set(redoIcon: string, undoIcon: string | null, newValue: any): Promise<Action> {
