...
class ActionStack {
  ...
  static async construct(mm: ModuleManager, db: Database, idKey: any) {
    ...
    actionStack.pointer = await PersistedPrimitive.construct(
      db,
      `${idKey}.pointer`,
      -1
    );
  ...
  async edit(_id: number, newAction: any) {
    let index = await this.items.findIndex(
      (a: { _id: number }) => a._id == _id
    );
  ...
  async doGroup(actionName: string, actions: Function) {
    ...
    await this.items.splice(
      this.pointer.get() + 1,
      this.items.length - this.pointer.get() - 1
    );
    ...
    let newActions = await this.items.filter(
      (a: { _id: number }) => a._id > actionID
    );
  ...
  async do(oid?: string, method?: string, args?: any[]) {
    ...
    try {
      ...
      // Check the action
      if (!action)
        throw new Error(`${oid}.${method}() did not return an action.`);
    } catch (ex: any) {
    ...
    action.redo.ts.elapsed =
      action.redo.ts.ended.getTime() -
      (action.redo.ts.started ?? new Date()).getTime();
    // Add the new action
    await this.add(action);
    // Return the result
